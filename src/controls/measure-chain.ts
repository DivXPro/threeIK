 ...
 * 注意必须用世界空间距离——模型可能有祖先缩放（如 Mixamo 模型的 Character 节点 scale=0.01，
...
export function measureChain(
  ...
  rootName: string,
  endName: string,
): { rootBone: Object3D; reach: number } | null {
  const rootBone = sceneRoot.getObjectByName(rootName);
  if (!rootBone) return null;

  const findDescendant = (node: Object3D): Object3D | null => {
