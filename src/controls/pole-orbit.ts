import { Camera, Mesh, MeshBasicMaterial, Object3D, Plane, Quaternion, Raycaster, SphereGeometry, TorusGeometry, Vector2, Vector3 } from 'three';
...
const _center = new Vector3(); // 环心 = 中骨关节世界位置
...
const Z = new Vector3(0, 0, 1); // torus 默认躺 XY 平面、轴 +Z；环面 ⊥ 链轴即 +Z→链轴

/**
 * pole 转向操纵器（环上带球）：环心 = 中骨关节（肘/膝），环面 ⊥「根骨→端球」链轴，
 * 球以固定半径待在环上——这个环就是肘/膝真实能转的轨迹（弯度不变时中骨关节绕链轴的圆）。
 * 拖球 = 球沿环滑（离环的拖动被吸回环上），肘/膝随球转向；球的位置完全由
 * 「环心 + 链轴 + 环上方向」逐帧推出，身体移动时自动跟随，无需携带偏移。
 * 纯位置控制点：不参与 W/E 操纵器模式切换，两种模式都常驻可用。
 * 暴露的 ball 即 TwoBoneIK 的 poleTarget（求解器只读其世界位置）。
 */
export class PoleOrbit extends Object3D {
  /** 环上的球（TwoBoneIK poleTarget / 引导线端点都读它） */
  readonly ball: Mesh;
  ...
  // 环心/链轴来源（bind 注入）：环心 = anchor 世界位置；链轴 = axisFrom→axisTo（端球而非端骨——
  ...
  /** 环上方向（世界系，持久状态）：每帧投影 ⊥ 当前链轴——链轴随手球拖动变化时方向平滑跟随不跳变 */
  private readonly dir = new Vector3(0, 0, 1);
  private dirHint: Vector3 | null = null; // 首帧前的初始方向来源（spec 位置或默认摆位）

  constructor(camera: Camera, dom: DragDom, options: { color?: number; ballRadius?: number; ringRadius?: number; dragControl?: DragControl } = {}) {
    super();
    this.camera = camera;
    this.dom = dom;
    this.dragControl = options.dragControl;
    this.ballRadius = options.ballRadius ?? 0.0225;
    this.ringRadius = options.ringRadius ?? 0.2;
    ...
    this.add(this.ball);
    this.ringMaterial = new MeshBasicMaterial({ color: options.color ?? 0xffcc00, depthTest: false, transparent: true, opacity: 0.4 });
    this.ringMesh = new Mesh(new TorusGeometry(1, 0.02, 8, 64), this.ringMaterial);
    this.ringMesh.scale.setScalar(this.ringRadius);
    ...
    this.onPointerMove = (e) => {
      ...
      // 吸回环面：命中点去轴向分量即环上方向（链轴几乎 ⊥ 视线时命中很远，方向仍然有效）
  ...
  /** 绑定环心/链轴来源：anchor = 中骨关节，链轴 = axisFrom（根骨）→ axisTo（端球） */
  ...
  setDirection(worldDir: Vector3): void {
    ...
    _local.copy(worldDir);
    _local.addScaledVector(_axis, -_local.dot(_axis));
    if (_local.lengthSq() < 1e-10) return;
    this.dir.copy(_local.normalize());
    this.place();
  }

  /** 环半径 = 球到环心的固定距离（原 poleRadius 语义） */
  ...
  /** 按持久状态落位：环心 = 中骨关节，环面 ⊥ 链轴，球 = 环心 + 半径 × 环上方向 */
  private place(): void {
    if (!this.frame(_center, _axis)) return;
    ...
    this.dir.normalize();
    // 世界 → 父局部写入（挂点即场景顶层时恒等）。_q = 本体的世界朝向（+Z→链轴）
    _q.setFromUnitVectors(Z, _axis); // 环面 ⊥ 链轴（环绕轴对称，roll 任意）
    if (this.parent) {
      this.position.copy(this.parent.worldToLocal(_local.copy(_center)));
      this.parent.getWorldQuaternion(_pq).invert();
      this.quaternion.copy(_pq).multiply(_q);
    } else {
      this.position.copy(_center);
      this.quaternion.copy(_q);
    }
    // 球是本体子节点：局部偏移 = 本体世界朝向的逆 ·（环上方向×半径）。
    // 注意要用 _q 的逆而不是父的逆——否则父带旋转时球会偏离环面
    this.ball.position.copy(this.dir).multiplyScalar(this.ringRadius).applyQuaternion(_pq.copy(_q).invert());
  }
